...
df2$TotVax <- as.integer(df2$TotVax) #doing some rounding
...
dd <- melt(sentAns, id=c("date"))
names(dd)[2] <- "Legend"

labels <- data.frame(unique(dd$Legend))
...
dd %>% 
  ...
  labs(title = "Average Sentament + Vaccinations + Deaths by Date",
       x = "Months",
       y = "Vaccination and Death Totals",
       caption = "Sentiment Score is 0-1 and Scaled for this graphic") +
  scale_y_continuous(labels=comma, 
                     sec.axis = sec_axis( ~./xm, name = "Sentiment Score",
