...
sent <- read.csv("C:/Users/justi/Documents/GitHub/7330-Term-Porject/Data Set/tweets_loc_sen0315.csv")
#View(sent)
#names(sent)
...
# scale to same as Total Vaccinations(00) by mult mean by df2[dim(df2)[1],2] and we will call that mulitplier 'xm'
xm <- df2[dim(df2)[1],2]
...
dd %>% 
  ...
  geom_line() +
  #geom_point() +
  scale_color_manual(values = c("blue", "black", "red", "coral", "green")) +
  labs(title = "Average Sentiment vs Total Covid-19 Vaccinations and Total Covid-19 Deaths in the USA",
       x = "Months",
       y = "Vaccination and Death Totals",
       caption = "Sentiment Score is 0-1 and Scaled for this graphic.
       Data thru March 14, 2021...extra padding is added to this graph for labels") +
  ...
  scale_x_date(limits = as.Date(c("2020-12-12", "2021-04-01"))) +
  directlabels::geom_dl(aes(label = Legend), method = list("last.polygons", cex=0.9)) +
  theme(axis.text.y.right = element_text(margin = unit(c(0, 0, 0, 0), "pt")))
